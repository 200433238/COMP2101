...
listyy=$(lshw -class network |  awk '/logical name:/{print $3}')
declare -a interface=($listyy)
len=${#interface[*]}
i=0
# Making a loop to process work in the array
while [ $i -lt $len ]
do
# Doing the setup to skip the loopback interfaces
    if [[ ${interface[$i]} = lo* ]] ; then continue ; fi
	    # Lines to place the output from the command’s to variable
