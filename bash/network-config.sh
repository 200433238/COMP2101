...
myhostname=$(hostname)
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
#finding interface name and lan address.
...
#using getent hosts and variable $lan-address, get the lan hostname.
external_ip=$(curl -s icanhazip.com)
#get external ip address from curl.
external_name=$(getent hosts $external_ip| awk '{print $2}')
#Getting external name  by looking into hosts file by $external-ip.
router_address=$(ip r s default| cut -d ' ' -f 3)
#finding the default router address.
router_hostname=$(getent hosts $router_address|awk '{print $2}')
#using getent host and $router-address, find router hostname.

#clean up the script by by moving rhe command output to separate variables.
cat <<EOF
Hostname        : $myhostname
