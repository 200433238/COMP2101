...
myargs=()
# Making variables to recognize the stat of the input
verbosemcheck=0
debugmcheck=0
...
while [ $# -gt 0 ]; do
  ...
  -d )
    ...
    [1-5] )
		...
		debugmcheck=$2
		shift
		# swifting the command line argument to proceed the upcoming element
		;;
    *)
...
# Using if to inspect the stat of variable
if [ $verbosemcheck = 1 ]; then
...
# Print out the myargs array with a label
# Using if to inspect the stat of variable
if [ $debugmcheck -gt 0 ]; then
  echo "Debug Mode is On with debug level $debugmcheck."
else
